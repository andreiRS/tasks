...
| `UNKNOWN_UUID` | A dep points at a task the graph validator can't resolve. The error names both uuids: the task carrying the bad edge and the missing target. The validator runs on every graph mutation and only sees *live* tasks, so this also fires when a live task depends on an **archived** task. | If it's the task you just touched, you used a stale or wrong id (check `tasks list --json`). If it names a *different* task, that task has a stale edge blocking all graph mutations. Note `tasks unlink <that-task> --depends-on <uuid>` does **not** work here, it rejects an unresolvable target with `target not found`. Instead use `tasks edit <that-task>` (exempt from the dirty guard) and clear the bad dep, e.g. with a scripted `$EDITOR` that rewrites the `deps:` line to `deps: []`, then retry. |
